...
function getScroll(lvl, n) {
    let scrolls = [];
    for (let i = 0; i < n; i++) {
        scrolls[i] = 'Scroll of ' + randomIndex(lvl);
    }
    return scrolls;   
};

function getWand(lvl, n) {
    let wands = [];
    for (let i = 0; i < n; i++) {
        wands[i] = 'Wand of ' + randomIndex(lvl);
    }
    return wands;
};
...
console.log(getScroll(second, 5));
console.log(getWand(second, 5));
